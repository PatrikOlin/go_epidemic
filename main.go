...
// simulation config
var width *int // number of cells on one side of the image
var numDays *int // number of days to simulate
var filename *string // name of data file

// epidemic simulation parameters
var rate *float64 // probability that infection occurs
var incubation *int // incubation time (time before being infectious)
var duration *int // how long disease lasts before fatality or recovery
...
func main() {
	...
	for t = 0; !endSim && (t < *numDays); t++ {
		...
		for n := range cells {
			// if cell is empty or not infected, go to next cell
